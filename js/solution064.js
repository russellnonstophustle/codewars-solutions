...
function calculateYears(principal, interest, tax, desired) {
    //   first check if principal is already at desired level, while() can do this
    //   if not add the interest accrued for the year and minus the tax on the interest
    //   check if it is at desired level, which the while() loop will do
    // if not repeat process while incrementing the years (make a year variable to return)
       let year=0;
