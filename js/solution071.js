...
function queueTime(customers, n) {
    //   create an array for each till
      const tills = new Array(n).fill(0);
    //   loop through the customers wait times to populate the tills arrays available
      for (let waitTime of customers) {
    // add the next wait time to the array (till) with the lowest wait time
        const lowestWaitTill = tills.indexOf(Math.min(...tills));
        tills[lowestWaitTill] += waitTime;
      }
    // return the till with the longest wait time
